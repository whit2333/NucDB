...
#-----------------------------------2.75 deg-----------------------------------! 
#----------------proton----------------!!--------------deuteron----------------! 
#  <x>     <Q2>   A_||  +-stat  +-syst     <x>     <Q2>   A_||  +-stat  +-syst   
def ProtonE155DataToDBList(line,exp,ang):
    vals=line.split()
    results=[]
    ...
    results.append(float(vals[2])-float(vals[3]))       #syst_error_low
    results.append(float(vals[2])+float(vals[3]))       #syst_error_high
    results.append(float(vals[2])-float(vals[4]))       #stat_error_low
    results.append(float(vals[2])+float(vals[4]))       #stat_error_high
    results.append("'p'")     #nucleon
    return(results)

#  <x>     <Q2>   A_||  +-stat  +-syst     <x>     <Q2>   A_||  +-stat  +-syst   
def DeuteronE155DataToDBList(line,exp,ang):
    vals=line.split()
    results=[]
    results.append(experiment) #experiment
    results.append(float(ang)) #angle
...
for line in datafile:
    ...
    res=DeuteronE155DataToDBList(line,experiment,2.75)
    sqlcmd="REPLACE INTO " + table + " SET "
    for n, v in enumerate(res):
        sqlcmd = sqlcmd + columnNames[n] + "=" + str(v) +", "
    sqlcmd = sqlcmd.strip(", ")
...
for line in datafile:
    ...
    print sqlcmd
    cursor.execute (sqlcmd)
    res=DeuteronE155DataToDBList(line,experiment,5.5)
    sqlcmd="REPLACE INTO " + table + " SET "
    for n, v in enumerate(res):
        sqlcmd = sqlcmd + columnNames[n] + "=" + str(v) +", "
    sqlcmd = sqlcmd.strip(", ")
    sqlcmd = sqlcmd + " ; "
    print sqlcmd
    cursor.execute (sqlcmd)# Close MySQL connection
cursor.close ()
conn.close ()
# close file
datafile.close()
...
for line in datafile:
    ...
    print sqlcmd
    cursor.execute (sqlcmd)
    res=DeuteronE155DataToDBList(line,experiment,10.5)
    sqlcmd="REPLACE INTO " + table + " SET "
    for n, v in enumerate(res):
        sqlcmd = sqlcmd + columnNames[n] + "=" + str(v) +", "
    sqlcmd = sqlcmd.strip(", ")
