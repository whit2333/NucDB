Int_t ratio_CLAS_5pass(){
   ...
   exp = dbman->GetExperiment("CLAS");
   if(!exp ) {
      exp = new NucDBExperiment("CLAS","CLAS");
   }
   ...
   std::ifstream file("experiments/inclusive/positron_electron_ratio/data_output/CLAS_5pass.dat");
   double xcol[10];
   while( !(file.eof()) ){
      for(int i = 0; i< 10;i++){
         file >> xcol[i] ;
