...
if __name__ == "__main__":
    ...
    if not g1He3 :
        g1He3 = NucDBMeasurement("g1He3","g_{1}^{3He}")
        experiment.AddMeasurement(g1He3)
    g1He3.ClearDataPoints()
    g1He3.AddRef(ref)

    extractor = JLABE01012Extractor2()
    extractor.SetMeasurement(g1He3)
    extractor.iQsq          = 0
    extractor.iW            = 1
    extractor.ixbjorken     = 2
    extractor.iValueRow     = 3
    extractor.istatErr      = 4
    extractor.isysErr       = 5
    extractor.SetInputFile("experiments/JLAB-E01012/jam_dataset_3He_g1.csv",1)
    extractor.fCurrentDataPoint.AddBinVariable(Xbjorken)
    extractor.fCurrentDataPoint.AddBinVariable(Qsq)
    extractor.fCurrentDataPoint.AddBinVariable(W)
    extractor.Initialize()
